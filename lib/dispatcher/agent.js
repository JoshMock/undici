...
class Agent extends DispatcherBase {
  constructor ({ factory = defaultFactory, maxOrigins = Infinity, connect, ...options } = {}) {
    ...
    const decrementConnections = (origin, err) => {
      ...
      if (result) {
        ...
        if (result.count <= 0) {
          ...
          if (err && err.code !== 'UND_ERR_INFO') {
            result.dispatcher.destroy(err)
          }
        }
      }
    }
    ...
    this[kOnDisconnect] = (origin, targets, err) => {
      const key = origin.origin
      console.log('kOnDisconnect', key, targets.length, err?.message)
      decrementConnections(key, err)
      this.emit('disconnect', key, [this, ...targets], err)
    }

    this[kOnConnectionError] = (origin, targets, err) => {
      const key = origin.origin
      console.log('kOnConnectionError', key, targets.length, err?.message)
      decrementConnections(key, err)
